...
export default function FAQ() {
  ...
  return (
    <section className="py-20 lg:py-28 mt-8 bg-snow relative overflow-hidden">
          ...
          <div className="relative bg-white rounded-2xl shadow-card overflow-hidden">
            ...
            <div className="absolute right-0 top-0 bottom-0 w-4 bg-gray-50 border-l border-gray-100">
              {faqs.map((_, index) => (
                <div
                  key={index}
                  className={`absolute right-0 w-full h-8 transition-colors ${openIndex === index ? 'bg-ignition-600' : 'bg-gray-200'
                    }`}
              ...
              {faqs.map((faq, index) => (
                ...
                >
                  ...
                  >
                    ...
                    >
                      <GiSpanner className={`w-5 h-5 transition-colors ${openIndex === index ? 'text-ignition-600' : 'text-steel-400'
                        }`} />
                    </motion.div>
                      ...
                      >
                        <div className="px-6 pb-6 pl-16">
                          ...
                          <div className="flex items-center gap-3 pt-3 border-t border-gray-100">
                            <span className="text-sm text-steel-400">Was this helpful?</span>
                            <div className="flex gap-2">
                              <motion.button
                                type="button"
                                onClick={() => trackFAQInteraction(faq.question, true)}
                                className="px-3 py-1 text-sm rounded-full bg-success/10 text-success hover:bg-success/20 transition-colors flex items-center gap-1"
                              ...
                              <motion.button
                                type="button"
                                onClick={() => trackFAQInteraction(faq.question, false)}
                                className="px-3 py-1 text-sm rounded-full bg-gray-100 text-steel-500 hover:bg-gray-200 transition-colors flex items-center gap-1"
                                whileHover={{ scale: 1.05 }}
                                whileTap={{ scale: 0.95 }}
                              >
                                👎 No
